...
static __global__ void flash_attn_tile_ext_f16(
    ...
    __shared__ half2 KV_tmp[FATTN_KQ_STRIDE_TILE_F16][Dp/2 + 1]; // Pad D to avoid memory bank conflicts.
...
#pragma unroll
    ...
    half2 VKQ[ncols/nwarps][(Dp/2)/WARP_SIZE] = {{{0.0f, 0.0f}}};

    // Load Q into shared memory with bounds checking for non-64-aligned D
    __shared__ half2 Q_h2[ncols][Dp/2];
#pragma unroll
    for (int j0 = 0; j0 < ncols; j0 += nwarps) {
...
#pragma unroll
            for (int k_KQ_0 = 0; k_KQ_0 < Dp/2; k_KQ_0 += WARP_SIZE) {
                const int k_KQ = k_KQ_0 + threadIdx.x;
                const bool load_valid = k_row_valid && (k_KQ < D/2);

                KV_tmp[i_KQ][k_KQ] = load_valid ? K_h2[(k_VKQ_0 + i_KQ)*stride_KV2 + k_KQ]
...
#pragma unroll
            for (int i0 = 0; i0 < Dp/2; i0 += WARP_SIZE) {
                ...
                KV_tmp[k][i] = load_valid ? V_h2[(k_VKQ_0 + k)*stride_KV2 + i]
                                          : make_half2(0.0f, 0.0f);
            }
        }
